...
describe("InMemoryLibraryService", () => {
  ...
  it("filters by albumId", async () => {
    const svc = new InMemoryLibraryService();
    const p1 = await svc.addPhoto({ libraryId: LIBRARY_ID, originalName: "a.jpg", dataUrl: "data:image/jpeg;base64,a" });
    const p2 = await svc.addPhoto({ libraryId: LIBRARY_ID, originalName: "b.jpg", dataUrl: "data:image/jpeg;base64,b" });
  ...
  it("throws when updating a non-existent photo", async () => {
    const svc = new InMemoryLibraryService();
    await expect(
      svc.updatePhoto("ghost-id", { isFavorite: true }),
